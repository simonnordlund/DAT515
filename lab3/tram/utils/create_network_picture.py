...
TRAM_URL_FILE = 'URL.json'  # given in lab3/files, replace with your own in bonus 2
...
def scaled_position(network):
    minlon, minlat, maxlon, maxlat = network.extreme_positions()
    size_x = maxlon - minlon
    scalefactor = len(network)/2  # heuristic
...
def network_graphviz(network, outfile=MY_GBG_SVG, positions=scaled_position):
    ...
    for stop in network.all_stops():
        ...
        x, y = network.stop_positions(stop)
        if positions:
            y, x = positions(network)((x, y))
        pos_x, pos_y = str(x), str(y)
        
        col = 'white'
            
        dot.node(stop, label=stop, shape='rectangle', pos=pos_x + ',' + pos_y +'!',
            fontsize='15pt', width='0.8', height='0.1',
            URL=stop_url(stop),
    ...
    for line in network.all_lines():
        ...
        for i in range(len(stops)-1):
            dot.edge(stops[i], stops[i+1],
                         color=gbg_linecolors[int(line)], penwidth=str(4))
